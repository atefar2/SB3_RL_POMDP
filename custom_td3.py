...
class CustomTD3(TD3):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        if hasattr(self.env, 'envs') and hasattr(self.env.envs[0], 'reward_type'):
            ...
            # Monitor-wrapped environment (most common case)
        ...
        if not hasattr(self, '_reward_type_logged'):
            print(f"🔧 CustomTD3: Detected reward_type='{env_reward_type}', L2_reg={'ENABLED' if use_l2_reg else 'DISABLED'}")
            if not use_l2_reg:
                print(f"🎯 POMDP Mode: Using entropy regularization instead of L2 smoothness penalty")
        ...
        for _ in range(gradient_steps):
            ...
            if self._n_updates % self.policy_delay == 0:
                ...
                if use_l2_reg:
                    ...
                    # Periodic debug logging for L2 regularization
                    if self._n_updates % 1000 == 0:
                        print(f"🔧 L2 Regularization Applied: smooth_loss={smooth_loss.item():.6f}, coef={self.action_reg_coef}")
